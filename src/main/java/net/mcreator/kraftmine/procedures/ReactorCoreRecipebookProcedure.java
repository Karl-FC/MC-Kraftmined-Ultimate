...
public class ReactorCoreRecipebookProcedure {
	public static void execute(Entity entity) {
		...
		if (new Object() {
		...
		}.hasRecipe(entity, ResourceLocation.parse("kraftmine:reactor_core")) == false) {
			if (entity instanceof Player _playerHasItem ? _playerHasItem.getInventory().contains(new ItemStack(Blocks.BEACON)) : false) {
				if (entity instanceof ServerPlayer _serverPlayer)
					_serverPlayer.awardRecipesByKey(Collections.singletonList(ResourceLocation.parse("kraftmine:reactor_core")));
			}
		}
	}
}
