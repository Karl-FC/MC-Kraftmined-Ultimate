				...
				for (Entity entityiterator : _entfound) {
					if (world instanceof Level _level) {
						if (!_level.isClientSide()) {
							_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("kraftmine:pingu.die")), SoundSource.HOSTILE, 1, 1);
						} else {
							_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("kraftmine:pingu.die")), SoundSource.HOSTILE, 1, 1, false);
						}
					}
					if (entityiterator instanceof PenguinbigEntity || entityiterator instanceof PenguinEntity) {
						for (int index0 = 0; index0 < (int) (4); index0++) {
							entityiterator.hurt(DamageSource.STARVE, 10);
