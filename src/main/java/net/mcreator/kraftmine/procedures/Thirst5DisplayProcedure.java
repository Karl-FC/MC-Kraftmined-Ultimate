...
public class Thirst5DisplayProcedure {
	...
	private static boolean execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		...
		}.checkGamemode(entity) && world.getLevelData().getGameRules().getBoolean(KraftmineModGameRules.HYDRATION) == true && entity.getData(KraftmineModVariables.PLAYER_VARIABLES).Thirstlevel > 8) {
			return true;
		}
		return false;
	}
