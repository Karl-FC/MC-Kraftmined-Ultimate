...
public class BuffedWeaponsProcedure {
	public static void execute(Entity entity) {
		...
		double Buff = 0;
		double BuffChance = 0;
		Buff = entity instanceof LivingEntity _livingEntity0 && _livingEntity0.getAttributes().hasAttribute(KmKombatModAttributes.BUFF_LEVEL) ? _livingEntity0.getAttribute(KmKombatModAttributes.BUFF_LEVEL).getBaseValue() : 0;
			...
			if (entity instanceof Skeleton) {
				{
					...
					if (_entity instanceof Player _player) {
						_player.getInventory().armor.set(2, new ItemStack(KmKombatModItems.QUIVER_CHESTPLATE.get()));
						_player.getInventory().setChanged();
					} else if (_entity instanceof LivingEntity _living) {
						_living.setItemSlot(EquipmentSlot.CHEST, new ItemStack(KmKombatModItems.QUIVER_CHESTPLATE.get()));
					}
				}
			}
		}
